...
def test_metrics_basic():
    ...
    # Test count_matches
    matches = metrics.count_matches(tuple(smart_preds[0]), tuple(actual_draws[0]))
    logger.info(f"Matches between {smart_preds[0]} and {actual_draws[0]}: {matches}")

    # Test calculate_hit_rate
    hits, total = metrics.calculate_hit_rate(smart_preds, actual_draws, 2)
    hit_rate = hits / total if total > 0 else 0.0
    logger.info(f"Hit rate for tier 2: {hit_rate:.4f} ({hits}/{total})")
