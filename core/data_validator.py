...
class DataValidator:
    ...
    def normalize_numbers_field(numbers_field: Any) -> List[int]:
        """Handle both "[1,2,3]" strings and [1,2,3] lists"""
        if isinstance(numbers_field, list):
            # Already a list, convert elements to int
            return [int(num) for num in numbers_field]
        
        elif isinstance(numbers_field, str):
            ...
            raise TypeError(f"Unsupported numbers field type: {type(numbers_field)}")
